...
// Extern theme instances (defined in lcd_display.cpp)
...
protected:
    ...
    // --- UI phát nhạc trên canvas ---
    lv_obj_t* music_root_ = nullptr;
    lv_obj_t* music_date_label_ = nullptr;
    lv_obj_t* music_title_label_ = nullptr;
    lv_obj_t* karaoke_line_current_ = nullptr;
    lv_obj_t* karaoke_line_next_ = nullptr;
    lv_obj_t* karaoke_highlight_ = nullptr;
...
protected:
    // Protected constructor (used by derived display types)
...
public:
    ...
    // Hàm nhận diện source từ music_info_ + trạng thái player
    DisplaySourceType DetectSourceFromInfo() const;

    ~LcdDisplay();

    // Emotion / chat / preview
    ...
    virtual void UpdateKaraokeProgress(float percent) override;
    virtual void ShowKaraokeUI();
    virtual void HideKaraokeUI();
};
