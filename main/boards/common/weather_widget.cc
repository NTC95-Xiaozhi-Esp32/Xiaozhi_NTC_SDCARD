...
static bool ParseOpenWeatherIconCode(const std::string& s, int* kind_out, bool* night_out) {
    ...
    if (!std::isdigit(c0) || !std::isdigit(c1) || (c2 != 'd' && c2 != 'n')) return false;
...
static lv_color_t WeatherIconColor(const std::string& code_or_name) {
    ...
    if (ParseOpenWeatherIconCode(code_or_name, &kind, &night)) {
        (void)night; 
        if (kind == 1) return COLOR_VIVID_YELLOW; // Clear
        if (kind == 2) return COLOR_VIVID_ORANGE; // Few clouds
        if (kind == 3 || kind == 4) return COLOR_VIVID_BLUE; // Clouds
        if (kind == 9 || kind == 10) return COLOR_VIVID_CYAN; // Rain
        if (kind == 11) return COLOR_VIVID_PINK; // Thunder
        if (kind == 13) return CLR_NIGHT_TXT_MAIN; // Snow (White)
        if (kind == 50) return COLOR_VIVID_GREEN; // Mist
        return CLR_NIGHT_TXT_SUB;
    }
    ...
    if (lower.find("thunder") != std::string::npos || lower.find("storm") != std::string::npos) return COLOR_VIVID_PINK;
    if (lower.find("rain") != std::string::npos || lower.find("drizzle") != std::string::npos) return COLOR_VIVID_CYAN;
    if (lower.find("snow") != std::string::npos) return CLR_NIGHT_TXT_MAIN;
    if (lower.find("wind") != std::string::npos || lower.find("mist") != std::string::npos ||
        lower.find("fog") != std::string::npos || lower.find("haze") != std::string::npos) return COLOR_VIVID_GREEN;
    if (lower == "sun" || lower.find("clear") != std::string::npos) return COLOR_VIVID_YELLOW;
    if (lower == "cloud_sun") return COLOR_VIVID_ORANGE;
    if (lower.find("cloud") != std::string::npos) return COLOR_VIVID_BLUE;
    return CLR_NIGHT_TXT_SUB;
}
...
static int64_t NormalizeUnixSeconds(int64_t t) {
    if (t > 2000000000000LL) return t / 1000LL;
    return t;
}
...
WeatherWidget::WeatherWidget(lv_obj_t* parent, WeatherService* weather_service)
    ...
    for (auto& p : lbl_clock_digits_) p = nullptr;
    for (auto& p : forecast_day_label_) p = nullptr;
    for (auto& p : forecast_icon_label_) p = nullptr;
    for (auto& p : forecast_temp_label_) p = nullptr;
...
void WeatherWidget::CreateDetailArc(lv_obj_t* parent,
    ...
    *arc_out = arc;
    ...
    // Màu text sẽ được update trong Theme Logic
    lv_obj_set_style_text_color(lbl, CLR_NIGHT_TXT_MAIN, 0);
...
const char* WeatherWidget::GetWeatherIconGlyph(const std::string& code_or_name) {
    ...
    if (code_or_name.size() == 3 && std::isdigit(static_cast<unsigned char>(code_or_name[0]))) {
        ...
        if (kind == 1) return night ? FONT_AWESOME_MOON : FONT_AWESOME_SUN;
        if (kind == 2) return night ? FONT_AWESOME_CLOUD_MOON : FONT_AWESOME_CLOUD_SUN;
        if (kind == 3 || kind == 4) return FONT_AWESOME_CLOUD;
        if (kind == 9 || kind == 10) return "\uf043"; // rain
        if (kind == 11) return "\uf0e7"; // bolt
        if (kind == 13) return FONT_AWESOME_SNOWFLAKE;
        if (kind == 50) return FONT_AWESOME_WIND;
        return FONT_AWESOME_CLOUD;
    }
    if (code_or_name == "sun") return FONT_AWESOME_SUN;
    if (code_or_name == "cloud_sun") return FONT_AWESOME_CLOUD_SUN;
    if (code_or_name == "rain") return "\uf043";
    if (code_or_name == "thunder") return "\uf0e7";
    if (code_or_name == "mist" || code_or_name == "wind") return FONT_AWESOME_WIND;
    if (code_or_name == "snowflake") return FONT_AWESOME_SNOWFLAKE;
    return FONT_AWESOME_CLOUD;
}
...
std::string WeatherWidget::NormalizeVietnameseForFont(const std::string& input) {
    ...
    // ===== A / Ă / Â =====
    ReplaceAllUtf8InPlace(out, "Á", "á"); ReplaceAllUtf8InPlace(out, "À", "à"); ReplaceAllUtf8InPlace(out, "Ả", "ả"); ReplaceAllUtf8InPlace(out, "Ã", "ã"); ReplaceAllUtf8InPlace(out, "Ạ", "ạ");
    ReplaceAllUtf8InPlace(out, "Ă", "ă"); ReplaceAllUtf8InPlace(out, "Ắ", "ắ"); ReplaceAllUtf8InPlace(out, "Ằ", "ằ"); ReplaceAllUtf8InPlace(out, "Ẳ", "ẳ"); ReplaceAllUtf8InPlace(out, "Ẵ", "ẵ"); ReplaceAllUtf8InPlace(out, "Ặ", "ặ");
    ReplaceAllUtf8InPlace(out, "Â", "â"); ReplaceAllUtf8InPlace(out, "Ấ", "ấ"); ReplaceAllUtf8InPlace(out, "Ầ", "ầ"); ReplaceAllUtf8InPlace(out, "Ẩ", "ẩ"); ReplaceAllUtf8InPlace(out, "Ẫ", "ẫ"); ReplaceAllUtf8InPlace(out, "Ậ", "ậ");
    // ===== D (Đ) =====
    ReplaceAllUtf8InPlace(out, "Đ", "đ");
    // ===== E / Ê =====
    ReplaceAllUtf8InPlace(out, "É", "é"); ReplaceAllUtf8InPlace(out, "È", "è"); ReplaceAllUtf8InPlace(out, "Ẻ", "ẻ"); ReplaceAllUtf8InPlace(out, "Ẽ", "ẽ"); ReplaceAllUtf8InPlace(out, "Ẹ", "ẹ");
    ReplaceAllUtf8InPlace(out, "Ê", "ê"); ReplaceAllUtf8InPlace(out, "Ế", "ế"); ReplaceAllUtf8InPlace(out, "Ề", "ề"); ReplaceAllUtf8InPlace(out, "Ể", "ể"); ReplaceAllUtf8InPlace(out, "Ễ", "ễ"); ReplaceAllUtf8InPlace(out, "Ệ", "ệ");
    // ===== I =====
    ReplaceAllUtf8InPlace(out, "Í", "í"); ReplaceAllUtf8InPlace(out, "Ì", "ì"); ReplaceAllUtf8InPlace(out, "Ỉ", "ỉ"); ReplaceAllUtf8InPlace(out, "Ĩ", "ĩ"); ReplaceAllUtf8InPlace(out, "Ị", "ị");
    // ===== O / Ô / Ơ =====
    ReplaceAllUtf8InPlace(out, "Ó", "ó"); ReplaceAllUtf8InPlace(out, "Ò", "ò"); ReplaceAllUtf8InPlace(out, "Ỏ", "ỏ"); ReplaceAllUtf8InPlace(out, "Õ", "õ"); ReplaceAllUtf8InPlace(out, "Ọ", "ọ");
    ReplaceAllUtf8InPlace(out, "Ô", "ô"); ReplaceAllUtf8InPlace(out, "Ố", "ố"); ReplaceAllUtf8InPlace(out, "Ồ", "ồ"); ReplaceAllUtf8InPlace(out, "Ổ", "ổ"); ReplaceAllUtf8InPlace(out, "Ỗ", "ỗ"); ReplaceAllUtf8InPlace(out, "Ộ", "ộ");
    ReplaceAllUtf8InPlace(out, "Ơ", "ơ"); ReplaceAllUtf8InPlace(out, "Ớ", "ớ"); ReplaceAllUtf8InPlace(out, "Ờ", "ờ"); ReplaceAllUtf8InPlace(out, "Ở", "ở"); ReplaceAllUtf8InPlace(out, "Ỡ", "ỡ"); ReplaceAllUtf8InPlace(out, "Ợ", "ợ");
    // ===== U / Ư =====
    ReplaceAllUtf8InPlace(out, "Ú", "ú"); ReplaceAllUtf8InPlace(out, "Ù", "ù"); ReplaceAllUtf8InPlace(out, "Ủ", "ủ"); ReplaceAllUtf8InPlace(out, "Ũ", "ũ"); ReplaceAllUtf8InPlace(out, "Ụ", "ụ");
    ReplaceAllUtf8InPlace(out, "Ư", "ư"); ReplaceAllUtf8InPlace(out, "Ứ", "ứ"); ReplaceAllUtf8InPlace(out, "Ừ", "ừ"); ReplaceAllUtf8InPlace(out, "Ử", "ử"); ReplaceAllUtf8InPlace(out, "Ữ", "ữ"); ReplaceAllUtf8InPlace(out, "Ự", "ự");
    // ===== Y =====
    ReplaceAllUtf8InPlace(out, "Ý", "ý"); ReplaceAllUtf8InPlace(out, "Ỳ", "ỳ"); ReplaceAllUtf8InPlace(out, "Ỷ", "ỷ"); ReplaceAllUtf8InPlace(out, "Ỹ", "ỹ"); ReplaceAllUtf8InPlace(out, "Ỵ", "ỵ");
    return out;
}
...
void WeatherWidget::CreateUI() {
    ...
    const int zoom_clock       = static_cast<int>(zoom_std * 0.70f);  
    ...
    // Màu mặc định, sẽ được cập nhật trong UpdateHeaderAndClock
    lv_obj_set_style_bg_color(container_, CLR_DAY_BG, 0);
    ...
    lv_obj_set_style_pad_ver(row_header, 0, 0); 
    ...
    lv_obj_set_style_text_color(label_wifi_icon_, CLR_NIGHT_TXT_SUB, 0);
    {
    ...
    lv_obj_set_style_text_color(label_bat_icon_, CLR_NIGHT_TXT_SUB, 0);
    {
        int level = 0;
        bool charging = false;
    ...
    lv_obj_set_style_text_color(label_full_date_, CLR_NIGHT_TXT_SUB, 0);
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        lv_obj_set_style_text_color(lbl_clock_digits_[i], CLR_NIGHT_TXT_MAIN, 0);
        ...
        if (i == 2 || i == 5) {
            ...
            lv_obj_set_style_text_color(lbl_clock_digits_[i], CLR_NIGHT_TXT_SUB, 0);
        } else {
    ...
    lv_obj_set_style_text_color(label_city_, CLR_NIGHT_TXT_MAIN, 0);
    ...
    lv_obj_set_style_text_color(label_main_temp_, CLR_NIGHT_TXT_MAIN, 0);
    ...
    lv_obj_set_style_text_color(label_main_desc_, CLR_NIGHT_TXT_SUB, 0);
    ...
    CreateDetailArc(group_details_, screen_width_, &arc_humid_, &label_humid_val_, COLOR_VIVID_GREEN);
    CreateDetailArc(group_details_, screen_width_, &arc_press_, &label_press_val_, COLOR_VIVID_PINK);
    CreateDetailArc(group_details_, screen_width_, &arc_wind_, &label_wind_val_, COLOR_VIVID_CYAN);

    // Row 6: Sunrise / Sunset
    ...
    lv_obj_set_style_text_color(label_sunrise_sunset_, COLOR_VIVID_YELLOW, 0);
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        lv_obj_set_style_text_color(lbl_d, CLR_NIGHT_TXT_SUB, 0);
        ...
        lv_obj_set_style_text_color(lbl_t, CLR_NIGHT_TXT_MAIN, 0);
...
void WeatherWidget::Show() {
    ...
    if (clock_timer_) esp_timer_start_periodic(clock_timer_, 1000000);
...
void WeatherWidget::Hide() {
    ...
    lv_obj_add_flag(container_, LV_OBJ_FLAG_HIDDEN);
    visible_ = false;
    if (clock_timer_) esp_timer_stop(clock_timer_);
}
...
void WeatherWidget::ClockTimerCallback(void* arg) {
    ...
    if (!widget || !widget->visible_) return;

    Application::GetInstance().Schedule([widget]() {
        if (!widget->visible_) return;
...
void WeatherWidget::UpdateHeaderAndClock() {
    if (!label_full_date_ || !lbl_clock_digits_[0]) return;

    // Use the same battery/network sources as the status bar.
    {
        const char* icon = Board::GetInstance().GetNetworkStateIcon();
        if (label_wifi_icon_) lv_label_set_text(label_wifi_icon_, icon ? icon : GetWifiIconGlyph(-50));
    }
    ...
    // ==========================================
    if (tm_local) {
        bool is_night = (tm_local->tm_hour >= 18 || tm_local->tm_hour < 6);
        ...
        if (prev_night_mode != (int)is_night) {
            ...
            lv_color_t c_dots = is_night ? CLR_NIGHT_DOTS : CLR_DAY_DOTS;

            // 1. Root BG
            lv_obj_set_style_bg_color(container_, c_bg, 0);

            // 2. Header Colors
            lv_obj_set_style_text_color(label_wifi_icon_, c_sub, 0);
            lv_obj_set_style_text_color(label_bat_icon_, c_sub, 0);
            lv_obj_set_style_text_color(label_full_date_, c_sub, 0);

            // 3. Clock Colors
            for (int i = 0; i < 8; ++i) {
                if (i == 2 || i == 5) {
                    lv_obj_set_style_text_color(lbl_clock_digits_[i], c_sub, 0);
                } else {
                    lv_obj_set_style_text_color(lbl_clock_digits_[i], c_main, 0);
                }
            }

            // 4. Main Weather Colors
            lv_obj_set_style_text_color(label_city_, c_main, 0);
            lv_obj_set_style_text_color(label_main_temp_, c_main, 0);
            lv_obj_set_style_text_color(label_main_desc_, c_sub, 0);
            
            // 5. Dots Color
            // Row 0 (dots top)
            lv_obj_t* row0 = lv_obj_get_child(main_col_, 0);
            if(row0) {
                 uint32_t child_cnt = lv_obj_get_child_cnt(row0);
                 for(uint32_t i=0; i<child_cnt; ++i) {
                     lv_obj_set_style_bg_color(lv_obj_get_child(row0, i), c_dots, 0);
                 }
            }
            // Row 8 (dots bottom) - index có thể thay đổi, nên lấy last child
            lv_obj_t* row_last = lv_obj_get_child(main_col_, lv_obj_get_child_cnt(main_col_) - 1);
            if(row_last) {
                 uint32_t child_cnt = lv_obj_get_child_cnt(row_last);
                 for(uint32_t i=0; i<child_cnt; ++i) {
                     lv_obj_set_style_bg_color(lv_obj_get_child(row_last, i), c_dots, 0);
                 }
            }

            // 6. Forecast Colors
            for(int i=0; i<5; ++i) {
                if(forecast_day_label_[i]) lv_obj_set_style_text_color(forecast_day_label_[i], c_sub, 0);
                if(forecast_temp_label_[i]) lv_obj_set_style_text_color(forecast_temp_label_[i], c_main, 0);
            }

            // 7. Details Labels (Số bên trong vòng tròn)
            if(label_humid_val_) lv_obj_set_style_text_color(label_humid_val_, c_main, 0);
            if(label_press_val_) lv_obj_set_style_text_color(label_press_val_, c_main, 0);
            if(label_wind_val_)  lv_obj_set_style_text_color(label_wind_val_, c_main, 0);
        }
    }
...
void WeatherWidget::UpdateUI(const WeatherData& weather) {
    ...
    if (!weather.valid) {
        ...
        if (label_main_icon_) {
            ...
            lv_obj_set_style_text_color(label_main_icon_, CLR_NIGHT_TXT_SUB, 0);
        }
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            if (forecast_icon_label_[i]) {
                ...
                lv_obj_set_style_text_color(forecast_icon_label_[i], CLR_NIGHT_TXT_SUB, 0);
            }
    ...
    // Forecast (5 days)
    for (int i = 0; i < 5; ++i) {
        if (!forecast_day_label_[i] || !forecast_icon_label_[i] || !forecast_temp_label_[i]) continue;
